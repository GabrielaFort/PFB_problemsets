...
for result in iprscan['results']:
  ...
  for match in result['matches']:
    ...
    if 'signature' in match:
      if 'entry' in match['signature'] and match['signature']['entry']:
#        entry =  match['signature']['entry']
        
        if 'accession' in match['signature']['entry']:
          acc = match['signature']['entry']['accession']
          if acc.startswith('IPR'):

            name=match['signature']['entry']['name']
            print(f'IPR: {acc} {name}')
...
print("==== INTERPRO2GO GO TERM Counts ===")
